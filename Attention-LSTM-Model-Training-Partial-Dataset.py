...
for run in runs:
    ...
    path = f"./Attention_LSTM/Partial FNC corpus/Model Training"
    ...
    for train_index, test_index in kf.split(X, y):
        fold_count += 1
        save_path = f"./Attention_LSTM/Partial FNC corpus/Model Training/fold{fold_count}"
        checkpoint_path = f"./Attention_LSTM/Partial FNC corpus/Model Training/fold{fold_count}/checkpoint"
        ...
        path = f'./fnc-1/dataset/test_dataset_agree_and_disagree_concat_representation.csv',
        format = 'csv',
        fields = [('text',CONCAT),('stance',LABEL),("index",INDEX)],
        skip_header = False)
        ...
        for epoch in range(run.epoch_num):
            ...
            torch.save({'epoch': epoch + 1,
                       ...
                       checkpoint_path + '/m-' + str(launchTimestamp) + '-' + str(epoch + 1) + '.pth.tar')
            #-------Test process-----#
            Attention_LSTM.eval()
            with torch.no_grad():
                m.begin_test_epoch()
