...
class Attention_StackLSTM_With_4_FC(nn.Module):
    def __init__(self, num_layers, dropout_rate, concat_embeddings, hidden_size, input_unit_num, unit_num,
        ...
        super(Attention_StackLSTM_With_4_FC, self).__init__()  # 调用父类的构造方法
        self.num_layers = num_layers
        self.attention_window = attention_window
        self.hidden_size = hidden_size
        self.bidirectional = bidirectional
        self.concat_embedding = nn.Embedding(len(concat_embeddings),
                                             embedding_dim)  # vocab_size词汇表大小,embedding_dim词嵌入维度
...
class Attention_StackLSTM_With_3_FC(nn.Module):
    def __init__(self, num_layers, dropout_rate, concat_embeddings, hidden_size, input_unit_num, unit_num,
                 embedding_dim, attention_window, num_class, trainable_embeddings,bidirectional):
        super(Attention_StackLSTM_With_3_FC, self).__init__()  # 调用父类的构造方法
        self.num_layers = num_layers
        self.attention_window = attention_window
        self.hidden_size = hidden_size
        self.bidirectional = bidirectional
        ...
        if self.bidirectional == True:
            ...
            self.fc1 = nn.Linear(input_unit_num + 2*hidden_size, unit_num)
            self.fc2 = nn.Linear(unit_num, unit_num)
            self.fc3 = nn.Linear(unit_num, num_class)
...
class Attention_StackLSTM_With_5_FC(nn.Module):
    def __init__(self, num_layers, dropout_rate, concat_embeddings, hidden_size, input_unit_num, unit_num,
                 embedding_dim, attention_window, num_class, trainable_embeddings,bidirectional):
        super(Attention_StackLSTM_With_5_FC, self).__init__()  # 调用父类的构造方法
        self.num_layers = num_layers
        self.attention_window = attention_window
        self.hidden_size = hidden_size
        self.bidirectional = bidirectional
        ...
        if self.bidirectional == True:
            ...
            self.fc5 = nn.Linear(unit_num, num_class)
...
for run in runs:
    ...
    path = f"./Attention_StackLSTM/Full fnc-1 corpus/5 Fold Train/Structure Tuning/FC_3_Bidirectional_{run.bidirectional}_Hidden_layer_{run.num_layer}/output"
    ...
    for train_index, test_index in kf.split(X, y):
        fold_count += 1
        save_path = f"./Attention_StackLSTM/Full fnc-1 corpus/5 Fold Train/Structure Tuning/FC_3_Bidirectional_{run.bidirectional}_Hidden_layer_{run.num_layer}/fold{fold_count}"
        ...
        if not os.path.exists("F:./.vector_cache"):
            os.mkdir("F:./.vector_cache")
        ...
        Attention_StackLSTM1 = Attention_StackLSTM_With_3_FC(num_layers=run.num_layer,
        ...
        Attention_StackLSTM1.apply(weigth_init)
        print(Attention_StackLSTM1)
        ...
        optimizer1 = optim.Adam(Attention_StackLSTM1.parameters(),lr=run.lr,betas=(0.9, 0.999), eps=1e-08, weight_decay=run.weight_decay)
        m.begin_run(save_path, run)
        for epoch in range(run.epoch_num):
            # -------Training process---#
            Attention_StackLSTM1.train()
            m.begin_train_epoch()
            for i, batch in enumerate(train_iter):
                ...
                preds = Attention_StackLSTM1(text, input_features)
...
for run in runs:
    fold_count = 0
    device = torch.device(run.device)
    path = f"./Attention_StackLSTM/Full fnc-1 corpus/5 Fold Train/Structure Tuning/FC_4_Bidirectional_{run.bidirectional}_Hidden_layer_{run.num_layer}/output"
    if not os.path.exists(path):
        os.makedirs(path)
    train_output = aggregate_result(path,run,5)
    val_output = aggregate_result(path,run,5)
    ...
    for train_index, test_index in kf.split(X, y):
        fold_count += 1
        save_path = f"./Attention_StackLSTM/Full fnc-1 corpus/5 Fold Train/Structure Tuning/FC_4_Bidirectional_{run.bidirectional}_Hidden_layer_{run.num_layer}/fold{fold_count}"
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        print('train_index:%s , test_index: %s ' % (train_index, test_index))
        Train = pd.DataFrame({"Text": X[train_index], "Stance": y[train_index], "Index": ind[train_index]})
        ...
        CONCAT.vocab.vectors.unk_init = init.xavier_uniform
        print(CONCAT.vocab.vectors.shape)
        Attention_StackLSTM2 = Attention_StackLSTM_With_4_FC(num_layers=run.num_layer,
                                                             dropout_rate=run.dropout_rate,
                                                             concat_embeddings=CONCAT.vocab.vectors,
                                                             hidden_size=100,
                                                             input_unit_num=features[run.features]['train'].shape[1],
                                                             ...
                                                             trainable_embeddings=run.trainable_embeddings,
                                                             bidirectional=run.bidirectional).to(device)
        Attention_StackLSTM2.apply(weigth_init)
        print(Attention_StackLSTM2)
        train_iter = data.BucketIterator(TrainDataset,
                                    batch_size = run.batch_size,
                                    device = device,
                                    sort_within_batch = True,
        ...
        train_features_set = features[run.features]['train']
        test_features_set = features[run.features]["test"]
        optimizer2 = optim.Adam(Attention_StackLSTM2.parameters(),lr=run.lr,betas=(0.9, 0.999), eps=1e-08, weight_decay=run.weight_decay)
        m.begin_run(save_path, run)
        for epoch in range(run.epoch_num):
            # -------Training process---#
            Attention_StackLSTM2.train()
            m.begin_train_epoch()
            for i, batch in enumerate(train_iter):
                text = batch.text.to(device)
                # print(text.shape) [75,128]
            ...
            for i, batch in enumerate(val_iter):
                ...
                if run.device == "cuda":
                    ...
                    index = batch.index.numpy().tolist()
                input_features = train_features_set[index].to(device)
                preds = Attention_StackLSTM2(text, input_features)
                loss = F.cross_entropy(preds, labels)
...
for run in runs:
    fold_count = 0
    device = torch.device(run.device)
    path = f"./Attention_StackLSTM/Full fnc-1 corpus/5 Fold Train/Structure Tuning/FC_5_Bidirectional_{run.bidirectional}_Hidden_layer_{run.num_layer}/output"
    if not os.path.exists(path):
        os.makedirs(path)
    train_output = aggregate_result(path,run,5)
    val_output = aggregate_result(path,run,5)
    ...
    for train_index, test_index in kf.split(X, y):
        fold_count += 1
        save_path = f"./Attention_StackLSTM/Full fnc-1 corpus/5 Fold Train/Structure Tuning/FC_5_Bidirectional_{run.bidirectional}_Hidden_layer_{run.num_layer}/fold{fold_count}"
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        print('train_index:%s , test_index: %s ' % (train_index, test_index))
        Train = pd.DataFrame({"Text": X[train_index], "Stance": y[train_index], "Index": ind[train_index]})
        ...
        CONCAT.vocab.vectors.unk_init = init.xavier_uniform
        print(CONCAT.vocab.vectors.shape)
        Attention_StackLSTM3 = Attention_StackLSTM_With_5_FC(num_layers=run.num_layer,
                                                             dropout_rate=run.dropout_rate,
                                                             concat_embeddings=CONCAT.vocab.vectors,
                                                             hidden_size=100,
                                                             input_unit_num=features[run.features]['train'].shape[1],
                                                             ...
                                                             trainable_embeddings=run.trainable_embeddings,
                                                             bidirectional=run.bidirectional).to(device)
        Attention_StackLSTM3.apply(weigth_init)
        print(Attention_StackLSTM3)
        train_iter = data.BucketIterator(TrainDataset,
                                    batch_size = run.batch_size,
                                    device = device,
                                    sort_within_batch = True,
        ...
        train_features_set = features[run.features]['train']
        test_features_set = features[run.features]["test"]
        optimizer3 = optim.Adam(Attention_StackLSTM3.parameters(),lr=run.lr,betas=(0.9, 0.999), eps=1e-08, weight_decay=run.weight_decay)
        m.begin_run(save_path, run)
        for epoch in range(run.epoch_num):
            # -------Training process---#
            Attention_StackLSTM3.train()
            m.begin_train_epoch()
            for i, batch in enumerate(train_iter):
                text = batch.text.to(device)
                # print(text.shape) [75,128]
                ...
                if run.device == "cuda":
                    ...
                    index = batch.index.numpy().tolist()
                input_features = train_features_set[index].to(device)
                preds = Attention_StackLSTM3(text, input_features)
                loss = F.cross_entropy(preds, labels)
