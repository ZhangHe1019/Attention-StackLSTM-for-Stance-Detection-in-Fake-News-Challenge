...
##########################Loading Features############################
root_path = r"F:\Individual_Project"
corpus_name = "fnc-1"
path = f'{root_path}/{corpus_name}/features'
...
for run in runs:
    ...
    path = f"./Attention_StackLSTM/Partial FNC corpus/Model Training/parameters"
    if not os.path.exists(path):
        os.makedirs(path)
    m = Prediction_Process(path=f"{path}",device=device)
    for train_index, test_index in kf.split(X, y):
        fold_count += 1
        checkpoint_path = f"./Attention_StackLSTM/Partial FNC corpus/Model Training/parameters/fold{fold_count}/checkpoint/fold{fold_count}.pth.tar"
