...
for run in runs:
    ...
    file_path = f"./Attention_StackLSTM/Full fnc-1 corpus/5 Fold Train/Hyperparameter Tuning/lr_{run.lr}_dropout_rate_{run.dropout_rate}/output"
    train_output = aggregate_result(file_path, run, 5)
    val_output = aggregate_result(file_path, run, 5)
    test_output = aggregate_result(file_path, run, 5)
    for train_index, test_index in kf.split(X, y):
        print("the number of training intances:",len(train_index))
        print("the number of validation instances",len(test_index))
        fold_count += 1
        save_path = f"./Attention_StackLSTM/Full fnc-1 corpus/5 Fold Train/Hyperparameter Tuning/lr_{run.lr}_dropout_rate_{run.dropout_rate}/fold{fold_count}"
        checkpoint_path = f"./Attention_StackLSTM/Full fnc-1 corpus/5 Fold Train/Hyperparameter Tuning/lr_{run.lr}_dropout_rate_{run.dropout_rate}/fold{fold_count}/checkpoint"
