...
def NMF_fit_all_incl_holdout_and_test(headlines, bodies, corpus_name):
    ...
    def get_all_data(head_and_body):
        features_dir = f"{corpus_name}/features"
    ...
    def get_features(head_and_body):
        ...
        if not (os.path.exists(features_dir + "/" + filename + ".pkl")):
            ...
            nfm = NMF(n_components=300, random_state=1)
...
def read_data(corpus_name, dataset_name):
    base_path = f'./{corpus_name}/dataset'
    # Extracting data
    if "fnc" in corpus_name:
...
def main(mode, corpus_name):
    ...
    Topic_features1 = torch.tensor(Topic_features1)
    Topic_features2 = torch.tensor(Topic_features2)
    Topic_features3 = torch.tensor(Topic_features3)
    Topic_features4 = torch.tensor(Topic_features4)
    topic_features = torch.cat([Topic_features1, Topic_features2, Topic_features3, Topic_features4], 1)
    with open(f"{filepath}/{mode}_topic_features.pkl", "wb") as handle:
        pickle.dump(topic_features, handle, protocol=4)
        handle.close()

if __name__ == '__main__':                                                                              
    parser = argparse.ArgumentParser(description='Extract topic features in mode and corpus')
