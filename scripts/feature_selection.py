...
from models.lstm_attention import StackLSTM_With_Attention_Network, weigth_init
...
def main():
    ...
    for run in runs:
        ...
        output_path = f"{result_path}/features_selection/{run.features}/output"
        if not os.path.exists(output_path):
            os.makedirs(output_path)
        train_output = aggregate_result(output_path, run, 5)
        val_output = aggregate_result(output_path, run, 5)
        test_output = aggregate_result(output_path, run, 5)

        # Cross-validation loop
        for train_index, test_index in kf.split(X, y):
            fold_count += 1
            save_path = f"{result_path}/features_selection/{run.features}/fold{fold_count}"
            checkpoint_path = f"{result_path}/features_selection/{run.features}/fold{fold_count}/checkpoint"
            ...
            # Initialize model
            Attention_StackLSTM = StackLSTM_With_Attention_Network(
                num_layers=run.num_layer, dropout_rate=run.dropout_rate, concat_embeddings=CONCAT.vocab.vectors,
                hidden_size=100, input_unit_num=features[run.features]['train'].shape[1], unit_num=600,
                embedding_dim=embedding_dim[run.word_embedding], attention_window=15, num_class=4,
                trainable_embeddings=run.trainable_embeddings, bidirectional=run.bidirectional
            ).to(device)
            Attention_StackLSTM.apply(weigth_init)

            # Initialize optimizer
            optimizer = optim.Adam(Attention_StackLSTM.parameters(), lr=run.lr, betas=(0.9, 0.999), eps=1e-08,
                                   weight_decay=run.weight_decay)

            train_features_set = features[run.features]['train']
            test_features_set = features[run.features]["test"]
            # Create iterators
            train_iter = data.BucketIterator(TrainDataset, batch_size=run.batch_size, device=device,
                                             sort_within_batch=True, sort_key=lambda x: len(x.text))
            val_iter = data.BucketIterator(ValDataset, batch_size=run.batch_size, device=device, sort_within_batch=True,
                                           sort_key=lambda x: len(x.text))
            test_iter = data.BucketIterator(TestDataset, batch_size=run.batch_size, device=device,
                                            sort_within_batch=False, sort_key=lambda x: len(x.text))
