...
def main():
    ...
    for run in runs:
        ...
        path = f"{result_path}/structure_improvement/bi_{run.bidirectional}_hidden_layer_{run.num_layer}_fc_{run.num_fc}/output"
        ...
        for train_index, test_index in kf.split(X, y):
            fold_count += 1
            save_path = f"{result_path}/structure_improvement/bi_{run.bidirectional}_hidden_layer_{run.num_layer}_fc_{run.num_fc}/fold{fold_count}"
            checkpoint_path = f"{result_path}/structure_improvement/bi_{run.bidirectional}_hidden_layer_{run.num_layer}_fc_{run.num_fc}/fold{fold_count}/checkpoint"
